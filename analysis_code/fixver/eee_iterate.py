...
# Using glob
dir_path = "/x6/spool/dylee/data/Storage/Fifth_data/root/WW/Fifth/*.root"
...
def Loop(file_list):
	...
	for arrays in tqdm(uproot.iterate(flist,branches)):
		...
		# Apply lepton cuts
		Tri_electron_mask = ak.num(PI_Electron) == 3
		no_muon_mask = ak.num(PI_Muon) == 0

		lepsel_mask = Tri_electron_mask & no_muon_mask
		...
		eee_triplet_idx = find_3lep(phosel_Electron,ak.ArrayBuilder()).snapshot()
		...
		try:
			Triple_electron = [ossf_Electron[eee_triplet_idx[idx]] for idx in "012"]
			...
			print("---> Electron triplet define done : {0}".format(len(Triple_eee.lep1.PT)))	
			...
			ptcut_Triple_eee = Triple_eee[Elenum_mask]
			ptcut_Photon = ossf_Photon[Elenum_mask]
			ptcut_MET = ossf_MET[Elenum_mask]

			print("---> Electron PT cut done : {0}".format(len(ak.flatten(ptcut_Triple_eee.lep1.PT))))	
			...
			METsel_Triple_eee = ptcut_Triple_eee[One_MET_mask]
			METsel_Photon = ptcut_Photon[One_MET_mask]		
			METsel_MET = ptcut_MET[One_MET_mask]		

			print("---> MET selection done : {0}".format(len(ak.flatten(METsel_Triple_eee.lep1.PT))))


			# Z mass window 
			leading_electron = vector.obj(pt=METsel_Triple_eee.lep1.PT, phi=METsel_Triple_eee.lep1.Phi, eta=METsel_Triple_eee.lep1.Eta, mass=0)
			subleading_electron = vector.obj(pt=METsel_Triple_eee.lep2.PT, phi=METsel_Triple_eee.lep2.Phi, eta=METsel_Triple_eee.lep2.Eta, mass=0)

			dilep = leading_electron + subleading_electron

			zmass_window = (np.abs(dilep.mass - 91.1876) < 10)
		
			zmass_Triple_eee = METsel_Triple_eee[zmass_window]
			zmass_Photon = METsel_Photon[zmass_window]
			zmass_MET = METsel_MET[zmass_window]

			print("---> Z mass window done : {0}".format(len(ak.flatten(zmass_Triple_eee.lep1.PT))))

		
			## Output Variable Define
			lep3_vec = vector.obj(pt=zmass_Triple_eee.lep3.PT, phi=zmass_Triple_eee.lep3.Phi)
			MET_vec = vector.obj(pt=zmass_MET.MET, phi=zmass_MET.Phi)
			MT_e = np.sqrt(2*(zmass_Triple_eee.lep3.PT)*(zmass_MET.MET)*(1-np.cos(np.abs(MET_vec.deltaphi(lep3_vec)))))	
		
			leading_electron = vector.obj(pt=zmass_Triple_eee.lep1.PT, phi=zmass_Triple_eee.lep1.Phi, eta=zmass_Triple_eee.lep1.Eta, mass=0)
			subleading_electron = vector.obj(pt=zmass_Triple_eee.lep2.PT, phi=zmass_Triple_eee.lep2.Phi, eta=zmass_Triple_eee.lep2.Eta, mass=0)
		
			third_electron = vector.obj(pt=zmass_Triple_eee.lep3.PT, phi=zmass_Triple_eee.lep3.Phi, eta=zmass_Triple_eee.lep3.Eta, mass=0)
			...
			lep1_F = ak.to_numpy(ak.flatten(zmass_Triple_eee.lep1.Charge * 1))		
			...
			lep2_F = ak.to_numpy(ak.flatten(zmass_Triple_eee.lep2.Charge * 1))	
			...
			lep3_F = ak.to_numpy(ak.flatten(zmass_Triple_eee.lep3.Charge * 1))
			...
			if len(histo) == 0:
				...
				histo['fifth'] = Triple_eee.lep1.PT
				histo['sixth'] = ak.flatten(ptcut_Triple_eee.lep1.PT)
				histo['seventh'] = ak.flatten(METsel_Triple_eee.lep1.PT)
				histo['eighth'] = ak.flatten(zmass_Triple_eee.lep1.PT)
				...
				histo['fifth'] = np.concatenate([histo['fifth'], Triple_eee.lep1.PT])
				histo['sixth'] = np.concatenate([histo['sixth'], ak.flatten(ptcut_Triple_eee.lep1.PT)])
				histo['seventh'] = np.concatenate([histo['seventh'], ak.flatten(METsel_Triple_eee.lep1.PT)])
				histo['eighth'] = np.concatenate([histo['eighth'], ak.flatten(zmass_Triple_eee.lep1.PT)])
...
np.save('/x6/spool/dylee/data/numpy/fixver/eee/{0}'.format(outname), histo, allow_pickle=True)
