...
# Using glob
dir_path = "/x6/spool/dylee/data/Storage/Fourth_data/root/WW/Fourth/*.root"
...
def Loop(file_list):
	...
	for arrays in tqdm(uproot.iterate(flist,branches)):
		...
		# Apply lepton cuts
		Di_muon_mask = ak.num(PI_Muon) == 2
		Single_electron_mask = ak.num(PI_Electron) == 1
		...
		mm_doublet_idx = find_2lep(phosel_Muon,ak.ArrayBuilder()).snapshot()
		...
		try:
			Di_muon = [ossf_Muon[mm_doublet_idx[idx]] for idx in "01"]
			...
			print("---> Muon doublet define done : {0}".format(len(Di_mu.lep1.PT)))	
			...
			ptcut_Di_mu = Di_mu[lepnum_mask]
			ptcut_Photon = ossf_Photon[lepnum_mask]
			ptcut_MET = ossf_MET[lepnum_mask]

			print("---> Lepton PT cut done : {0}".format(len(ak.flatten(ptcut_Di_mu.lep1.PT))))	
			...
			METsel_Di_mu = ptcut_Di_mu[One_MET_mask]
			METsel_Photon = ptcut_Photon[One_MET_mask]		
			METsel_MET = ptcut_MET[One_MET_mask]		

			print("---> MET selection done : {0}".format(len(ak.flatten(METsel_Di_mu.lep1.PT))))


			# Z mass window 
			leading_muon = vector.obj(pt=METsel_Di_mu.lep1.PT, phi=METsel_Di_mu.lep1.Phi, eta=METsel_Di_mu.lep1.Eta, mass=0)
			subleading_muon = vector.obj(pt=METsel_Di_mu.lep2.PT, phi=METsel_Di_mu.lep2.Phi, eta=METsel_Di_mu.lep2.Eta, mass=0)

			dilep = leading_muon + subleading_muon

			zmass_window = (np.abs(dilep.mass - 91.1876) < 10)
			
			zmass_Di_mu = METsel_Di_mu[zmass_window]
			zmass_Photon = METsel_Photon[zmass_window]
			zmass_MET = METsel_MET[zmass_window]

			print("---> Z mass window done : {0}".format(len(ak.flatten(zmass_Di_mu.lep1.PT))))

			
			## Output Variable Define
			lep3_vec = vector.obj(pt=zmass_Di_mu.lep3.PT, phi=zmass_Di_mu.lep3.Phi)
			MET_vec = vector.obj(pt=zmass_MET.MET, phi=zmass_MET.Phi)
			MT_e = np.sqrt(2*(zmass_Di_mu.lep3.PT)*(zmass_MET.MET)*(1-np.cos(np.abs(MET_vec.deltaphi(lep3_vec)))))	
			
			leading_muon = vector.obj(pt=zmass_Di_mu.lep1.PT, phi=zmass_Di_mu.lep1.Phi, eta=zmass_Di_mu.lep1.Eta, mass=0)
			subleading_muon = vector.obj(pt=zmass_Di_mu.lep2.PT, phi=zmass_Di_mu.lep2.Phi, eta=zmass_Di_mu.lep2.Eta, mass=0)	
			third_electron = vector.obj(pt=zmass_Di_mu.lep3.PT, phi=zmass_Di_mu.lep3.Phi, eta=zmass_Di_mu.lep3.Eta, mass=0)
			...
			lep1_F = ak.to_numpy(ak.flatten(zmass_Di_mu.lep1.Charge * 2))		
			...
			lep2_F = ak.to_numpy(ak.flatten(zmass_Di_mu.lep2.Charge * 2))	
			...
			lep3_F = ak.to_numpy(ak.flatten(zmass_Di_mu.lep3.Charge * 1))
			...
			if len(histo) == 0:
				...
				histo['fifth'] = Di_mu.lep1.PT
				histo['sixth'] = ak.flatten(ptcut_Di_mu.lep1.PT)
				histo['seventh'] = ak.flatten(METsel_Di_mu.lep1.PT)
				histo['eighth'] = ak.flatten(zmass_Di_mu.lep1.PT)

			else:
				histo['diele_mass'] = np.concatenate([histo['diele_mass'], dimu_mass])
				...
				histo['fifth'] = np.concatenate([histo['fifth'], Di_mu.lep1.PT])
				histo['sixth'] = np.concatenate([histo['sixth'], ak.flatten(ptcut_Di_mu.lep1.PT)])
				histo['seventh'] = np.concatenate([histo['seventh'], ak.flatten(METsel_Di_mu.lep1.PT)])
				histo['eighth'] = np.concatenate([histo['eighth'], ak.flatten(zmass_Di_mu.lep1.PT)])
...
np.save('/x6/spool/dylee/data/numpy/fixver/emm/{0}'.format(outname), histo, allow_pickle=True)
