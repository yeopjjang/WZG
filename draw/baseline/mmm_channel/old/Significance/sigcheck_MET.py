...
for i in range(0, 200, 1):
	...
	WZG_mask = (WZG.MET_pt > i) #& (ht_tt > j)
	WZG = WZG[WZG_mask]
		
	WG_mask = (WG.MET_pt > i) #& (ht_ttV > j)
	WG = WG[WG_mask]

	ZG_mask = (ZG.MET_pt > i) #& (ht_dy > j)
	ZG = ZG[ZG_mask]

	WW_mask = (WW.MET_pt > i) #& (ht_ww > j)
	WW = WW[WW_mask]

	WZ_mask = (WZ.MET_pt > i) #& (ht_wz > j)
	WZ = WZ[WZ_mask]

	ZZ_mask = (ZZ.MET_pt > i) #& (ht_zz > j)
	ZZ = ZZ[ZZ_mask]

	WWW_mask = (WWW.MET_pt > i) #& (ht_st > j)
	WWW = WWW[WWW_mask]

	WWZ_mask = (WWZ.MET_pt > i) #& (ht_dm > j)
	WWZ = WWZ[WWZ_mask]

	WZZ_mask = (WZZ.MET_pt > i) #& (ht_dm > j)
	WZZ = WZZ[WZZ_mask]

	ZZZ_mask = (ZZZ.MET_pt > i) #& (ht_dm > j)
	ZZZ = ZZZ[ZZZ_mask]

	ttbarG_mask = (ttbarG.MET_pt > i) #& (ht_dm > j)
...
plt.plot(x1,y1, color='darkred', linewidth=3)
plt.title("MET", fontsize=30, loc='center')
