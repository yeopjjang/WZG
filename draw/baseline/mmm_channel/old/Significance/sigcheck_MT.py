...
for i in range(0, 200, 1):
	...
	WZG_mask = (WZG.MT > i) #& (ht_tt > j)
	WZG = WZG[WZG_mask]
		
	WG_mask = (WG.MT > i) #& (ht_ttV > j)
	WG = WG[WG_mask]

	ZG_mask = (ZG.MT > i) #& (ht_dy > j)
	ZG = ZG[ZG_mask]

	WW_mask = (WW.MT > i) #& (ht_ww > j)
	WW = WW[WW_mask]

	WZ_mask = (WZ.MT > i) #& (ht_wz > j)
	WZ = WZ[WZ_mask]

	ZZ_mask = (ZZ.MT > i) #& (ht_zz > j)
	ZZ = ZZ[ZZ_mask]

	WWW_mask = (WWW.MT > i) #& (ht_st > j)
	WWW = WWW[WWW_mask]

	WWZ_mask = (WWZ.MT > i) #& (ht_dm > j)
	WWZ = WWZ[WWZ_mask]

	WZZ_mask = (WZZ.MT > i) #& (ht_dm > j)
	WZZ = WZZ[WZZ_mask]

	ZZZ_mask = (ZZZ.MT > i) #& (ht_dm > j)
	ZZZ = ZZZ[ZZZ_mask]

	ttbarG_mask = (ttbarG.MT > i) #& (ht_dm > j)
...
plt.plot(x1,y1, color='darkgoldenrod', linewidth=3)
plt.title("$M_{T}$", fontsize=30, loc='center')
