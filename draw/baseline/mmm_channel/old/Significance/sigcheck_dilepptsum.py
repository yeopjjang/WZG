...
for i in range(0, 200, 1):
	...
	WZG_mask = (WZG.Dilepptsum > i) #& (ht_tt > j)
	WZG = WZG[WZG_mask]
		
	WG_mask = (WG.Dilepptsum > i) #& (ht_ttV > j)
	WG = WG[WG_mask]

	ZG_mask = (ZG.Dilepptsum > i) #& (ht_dy > j)
	ZG = ZG[ZG_mask]

	WW_mask = (WW.Dilepptsum > i) #& (ht_ww > j)
	WW = WW[WW_mask]

	WZ_mask = (WZ.Dilepptsum > i) #& (ht_wz > j)
	WZ = WZ[WZ_mask]

	ZZ_mask = (ZZ.Dilepptsum > i) #& (ht_zz > j)
	ZZ = ZZ[ZZ_mask]

	WWW_mask = (WWW.Dilepptsum > i) #& (ht_st > j)
	WWW = WWW[WWW_mask]

	WWZ_mask = (WWZ.Dilepptsum > i) #& (ht_dm > j)
	WWZ = WWZ[WWZ_mask]

	WZZ_mask = (WZZ.Dilepptsum > i) #& (ht_dm > j)
	WZZ = WZZ[WZZ_mask]

	ZZZ_mask = (ZZZ.Dilepptsum > i) #& (ht_dm > j)
	ZZZ = ZZZ[ZZZ_mask]

	ttbarG_mask = (ttbarG.Dilepptsum > i) #& (ht_dm > j)
...
plt.plot(x1,y1, color='orangered', linewidth=3)
plt.title("Di-lepton $P_{T}$ Sum", fontsize=30, loc='center')
plt.xlabel("Cut Value | 1 GeV",fontsize=15, loc='center')
plt.ylabel("Expected Significance",fontsize=15, loc='center')
plt.savefig("Dilepptsum")
plt.show()
