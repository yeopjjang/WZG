...
for i in range(0, 300, 1):
	...
	WZG_mask = (WZG.lep1_E > i) #& (ht_tt > j)
	WZG = WZG[WZG_mask]
		
	WG_mask = (WG.lep1_E > i) #& (ht_ttV > j)
	WG = WG[WG_mask]

	ZG_mask = (ZG.lep1_E > i) #& (ht_dy > j)
	ZG = ZG[ZG_mask]

	WW_mask = (WW.lep1_E > i) #& (ht_ww > j)
	WW = WW[WW_mask]

	WZ_mask = (WZ.lep1_E > i) #& (ht_wz > j)
	WZ = WZ[WZ_mask]

	ZZ_mask = (ZZ.lep1_E > i) #& (ht_zz > j)
	ZZ = ZZ[ZZ_mask]

	WWW_mask = (WWW.lep1_E > i) #& (ht_st > j)
	WWW = WWW[WWW_mask]

	WWZ_mask = (WWZ.lep1_E > i) #& (ht_dm > j)
	WWZ = WWZ[WWZ_mask]

	WZZ_mask = (WZZ.lep1_E > i) #& (ht_dm > j)
	WZZ = WZZ[WZZ_mask]

	ZZZ_mask = (ZZZ.lep1_E > i) #& (ht_dm > j)
	ZZZ = ZZZ[ZZZ_mask]

	ttbarG_mask = (ttbarG.lep1_E > i) #& (ht_dm > j)
...
plt.plot(x1,y1, color='steelblue', linewidth=3)
plt.title("Leading Lepton Energy", fontsize=30, loc='center')
