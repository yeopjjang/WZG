...
# Data load
Data = '/x4/cms/dylee/Delphes/analysis/ntuple/mmm_channel.npy'

mmm_channel = np.load(Data,allow_pickle=True)[()]
...
for i in range(0, 300, 1):
	...
	WZG_mask = (WZG.Pho_pt > i) #& (ht_tt > j)
	WZG = WZG[WZG_mask]
		
	WG_mask = (WG.Pho_pt > i) #& (ht_ttV > j)
	WG = WG[WG_mask]

	ZG_mask = (ZG.Pho_pt > i) #& (ht_dy > j)
	ZG = ZG[ZG_mask]

	WW_mask = (WW.Pho_pt > i) #& (ht_ww > j)
	WW = WW[WW_mask]

	WZ_mask = (WZ.Pho_pt > i) #& (ht_wz > j)
	WZ = WZ[WZ_mask]

	ZZ_mask = (ZZ.Pho_pt > i) #& (ht_zz > j)
	ZZ = ZZ[ZZ_mask]

	WWW_mask = (WWW.Pho_pt > i) #& (ht_st > j)
	WWW = WWW[WWW_mask]

	WWZ_mask = (WWZ.Pho_pt > i) #& (ht_dm > j)
	WWZ = WWZ[WWZ_mask]

	WZZ_mask = (WZZ.Pho_pt > i) #& (ht_dm > j)
	WZZ = WZZ[WZZ_mask]

	ZZZ_mask = (ZZZ.Pho_pt > i) #& (ht_dm > j)
	ZZZ = ZZZ[ZZZ_mask]

	ttbarG_mask = (ttbarG.Pho_pt > i) #& (ht_dm > j)
	...
	if 0 <= sig <= 1000:
		...
		Photon_pt_x.append(i)
		Photon_pt_y.append(sig)
		print("# sig: {0} # bkg: {1} # sigma: {2} # Cut: {3}".format(signal,bkg,sig,i))

print("Max sig : {0}".format(max(Photon_pt_y)))	

# Draw hist
plt.figure(figsize=(8,8))
plt.style.use(hep.style.CMS)
plt.plot(Photon_pt_x,Photon_pt_y, color='dimgray', linewidth=3)
plt.title("Photon $P_{T}$", fontsize=30, loc='center')
