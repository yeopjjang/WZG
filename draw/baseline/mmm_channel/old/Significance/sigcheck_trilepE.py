...
for i in range(0, 500, 1):
	...
	WZG_mask = (WZG.Trilep_E > i) #& (ht_tt > j)
	WZG = WZG[WZG_mask]
		
	WG_mask = (WG.Trilep_E > i) #& (ht_ttV > j)
	WG = WG[WG_mask]

	ZG_mask = (ZG.Trilep_E > i) #& (ht_dy > j)
	ZG = ZG[ZG_mask]

	WW_mask = (WW.Trilep_E > i) #& (ht_ww > j)
	WW = WW[WW_mask]

	WZ_mask = (WZ.Trilep_E > i) #& (ht_wz > j)
	WZ = WZ[WZ_mask]

	ZZ_mask = (ZZ.Trilep_E > i) #& (ht_zz > j)
	ZZ = ZZ[ZZ_mask]

	WWW_mask = (WWW.Trilep_E > i) #& (ht_st > j)
	WWW = WWW[WWW_mask]

	WWZ_mask = (WWZ.Trilep_E > i) #& (ht_dm > j)
	WWZ = WWZ[WWZ_mask]

	WZZ_mask = (WZZ.Trilep_E > i) #& (ht_dm > j)
	WZZ = WZZ[WZZ_mask]

	ZZZ_mask = (ZZZ.Trilep_E > i) #& (ht_dm > j)
	ZZZ = ZZZ[ZZZ_mask]

	ttbarG_mask = (ttbarG.Trilep_E > i) #& (ht_dm > j)
...
plt.plot(x1,y1, color='chartreuse', linewidth=3)
plt.title("Tri-lepton Energy", fontsize=30, loc='center')
plt.xlabel("Cut Value | 1 GeV",fontsize=15, loc='center')
plt.ylabel("Expected Significance",fontsize=15, loc='center')
plt.savefig("Trilep_E")
plt.show()
