...
for i in range(0, 400, 1):
	...
	WZG_mask = (WZG.Trilepphoptsum > i) #& (ht_tt > j)
	WZG = WZG[WZG_mask]
		
	WG_mask = (WG.Trilepphoptsum > i) #& (ht_ttV > j)
	WG = WG[WG_mask]

	ZG_mask = (ZG.Trilepphoptsum > i) #& (ht_dy > j)
	ZG = ZG[ZG_mask]

	WW_mask = (WW.Trilepphoptsum > i) #& (ht_ww > j)
	WW = WW[WW_mask]

	WZ_mask = (WZ.Trilepphoptsum > i) #& (ht_wz > j)
	WZ = WZ[WZ_mask]

	ZZ_mask = (ZZ.Trilepphoptsum > i) #& (ht_zz > j)
	ZZ = ZZ[ZZ_mask]

	WWW_mask = (WWW.Trilepphoptsum > i) #& (ht_st > j)
	WWW = WWW[WWW_mask]

	WWZ_mask = (WWZ.Trilepphoptsum > i) #& (ht_dm > j)
	WWZ = WWZ[WWZ_mask]

	WZZ_mask = (WZZ.Trilepphoptsum > i) #& (ht_dm > j)
	WZZ = WZZ[WZZ_mask]

	ZZZ_mask = (ZZZ.Trilepphoptsum > i) #& (ht_dm > j)
	ZZZ = ZZZ[ZZZ_mask]

	ttbarG_mask = (ttbarG.Trilepphoptsum > i) #& (ht_dm > j)
