...
for i in range(0, 400, 1):
	...
	WZG_mask = (WZG.Trilepptsum > i) #& (ht_tt > j)
	WZG = WZG[WZG_mask]
		
	WG_mask = (WG.Trilepptsum > i) #& (ht_ttV > j)
	WG = WG[WG_mask]

	ZG_mask = (ZG.Trilepptsum > i) #& (ht_dy > j)
	ZG = ZG[ZG_mask]

	WW_mask = (WW.Trilepptsum > i) #& (ht_ww > j)
	WW = WW[WW_mask]

	WZ_mask = (WZ.Trilepptsum > i) #& (ht_wz > j)
	WZ = WZ[WZ_mask]

	ZZ_mask = (ZZ.Trilepptsum > i) #& (ht_zz > j)
	ZZ = ZZ[ZZ_mask]

	WWW_mask = (WWW.Trilepptsum > i) #& (ht_st > j)
	WWW = WWW[WWW_mask]

	WWZ_mask = (WWZ.Trilepptsum > i) #& (ht_dm > j)
	WWZ = WWZ[WWZ_mask]

	WZZ_mask = (WZZ.Trilepptsum > i) #& (ht_dm > j)
	WZZ = WZZ[WZZ_mask]

	ZZZ_mask = (ZZZ.Trilepptsum > i) #& (ht_dm > j)
	ZZZ = ZZZ[ZZZ_mask]

	ttbarG_mask = (ttbarG.Trilepptsum > i) #& (ht_dm > j)
...
plt.plot(x1,y1, color='turquoise', linewidth=3)
plt.title("Tri-lepton $P_{T}$ Sum", fontsize=30, loc='center')
plt.xlabel("Cut Value | 1 GeV",fontsize=15, loc='center')
plt.ylabel("Expected Significance",fontsize=15, loc='center')
plt.savefig("Trilepptsum")
plt.show()
