...
# Histogram Draw
stak = [ZZZ_Trilep_E, WZZ_Trilep_E, WWZ_Trilep_E, WWW_Trilep_E, ZZ_Trilep_E, WZ_Trilep_E, ttbarG_Trilep_E, WW_Trilep_E, ZG_Trilep_E, WG_Trilep_E]
...
plt.hist(WZG_Trilep_E, range=(0, 3000), bins=10, weights=scales["WZG"], color='red', histtype='step', label='WZG(signal)', linewidth=3)
plt.hist(stak, range=(0, 3000), alpha=0.7, weights=we, bins=10, color=co, label=la, stacked=True,edgecolor='black', linewidth=0.5)
plt.title("$\sqrt{s}=14 TeV, L=3000 fb^{-1}$", fontsize=13, loc='right')
plt.xlim(0, 3000)
plt.ylim(0.001, 10000)
plt.text(20, 4000, '(mmm channel)', fontsize=20)
plt.xlabel("Tri-lepton Energy [GeV]",fontsize=15, loc='center')
plt.ylabel("Expected Number of Events | 300 GeV",fontsize=15, loc='center')
plt.legend(fontsize=15, loc='upper right')
plt.yscale('log')
plt.savefig("mmm_Trilep_E")
plt.show()
