...
#		process_columns = channel['{0}'.format(process)].fields
		...
		if process == 'WZG':
			...
			y = np.ones(len(channel['{0}'.format(process)])) * 2
			evt = EventDict['ZG']
			xsec = xsecDict['ZG']
		elif process == 'WW':
			y = np.ones(len(channel['{0}'.format(process)])) * 3
			evt = EventDict['WW']
			xsec = xsecDict['WW']
		elif process == 'WZ':
			y = np.ones(len(channel['{0}'.format(process)])) * 4
			evt = EventDict['WZ']
			xsec = xsecDict['WZ']
		elif process == 'ZZ':
			y = np.ones(len(channel['{0}'.format(process)])) * 5
			evt = EventDict['ZZ']
			xsec = xsecDict['ZZ']
		elif process == 'WWW':
			y = np.ones(len(channel['{0}'.format(process)])) * 6
			evt = EventDict['WWW']
			xsec = xsecDict['WWW']
		elif process == 'WWZ':
			y = np.ones(len(channel['{0}'.format(process)])) * 7
			evt = EventDict['WWZ']
			xsec = xsecDict['WWZ']
		elif process == 'WZZ':
			y = np.ones(len(channel['{0}'.format(process)])) * 8
			evt = EventDict['WZZ']
			xsec = xsecDict['WZZ']
		elif process == 'ZZZ':
			y = np.ones(len(channel['{0}'.format(process)])) * 9
			evt = EventDict['ZZZ']
			xsec = xsecDict['ZZZ']
		else:
			y = np.ones(len(channel['{0}'.format(process)])) * 10
