...
## Read ntuple
infile = "/x4/cms/dylee/Delphes/ML/Node06_Machine_Learning/fixver_ML/data/emm_binary.h5"
...
# Train data
train_x = train_df.iloc[:, 3:-3].values
train_y = train_df.iloc[:,[0]].values 
train_w = train_df.iloc[:, [-3]].values

# Validation data
val_x = val_df.iloc[:, 3:-3].values 
val_y = val_df.iloc[:,[0]].values 
val_w = val_df.iloc[:, [-3]].values

test_df = test_df.reset_index(drop=True)

# Test data
test_x = test_df.iloc[:, 3:-3].values 
test_y = test_df.iloc[:,[0]].values 
test_w = test_df.iloc[:, [-3]].values
...
test_df.to_hdf('emm_testset.h5', key = 'df', mode = 'w')
